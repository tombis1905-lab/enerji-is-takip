...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string; odemeId: string }> }) {
  const g = await guard(req)
  if (!g.ok) return g.res
  const { odemeId } = await params
  await prisma.cariOdeme.delete({ where: { id: odemeId } }).catch(() => null)
