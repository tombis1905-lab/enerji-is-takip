...
// Çalışanın o an açık olan şirket kaydını, yeni bir şirkete geçmeden kapatır
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const acikKayit = await prisma.personelSirket.findFirst({
    where: { calisanId: id, bitisTarihi: null },
  })

  if (!acikKayit) {
    return NextResponse.json({ error: 'Bu çalışanın şu an açık bir şirket kaydı yok' }, { status: 400 })
  }
