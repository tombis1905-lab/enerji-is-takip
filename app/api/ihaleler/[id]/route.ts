...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.isAdi !== undefined && !body.isAdi?.trim()) {
    return NextResponse.json({ error: 'İşin adı boş olamaz' }, { status: 400 })
  }
  if (body.kurum !== undefined && !body.kurum?.trim()) {
    return NextResponse.json({ error: 'Kurum boş olamaz' }, { status: 400 })
  }

  try {
    const ihale = await prisma.ihale.update({ where: { id }, data: buildData(body) })
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params
  await prisma.ihale.delete({ where: { id } })
